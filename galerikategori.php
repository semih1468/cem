...
<section class="blog-page section-space">
    <div class="container">
        <div class="row gutter-y-30">
            ...
            $sayfada = 6;
            $sorgu = $db->prepare('select * from resim_kategori where  dilkod=:dilkod and ust_id=:ust_id order by resimkategori_id desc');
            $sorgu->execute(array(
            ...
            $hizmetsor = $db->prepare("SELECT * from resim_kategori where dilkod=:dilkod and ust_id=:ust_id order by resimkategori_id asc limit  $limit1 ,$sayfada  ");
...
"
                        >
                            <img style="height: 150px;object-fit: contain"
                                 src="upload/urun/<?php echo $blogcek['resimkategori_resim'] ?>"
                                 alt="<?php echo $blogcek['resimkategori_baslik'] ?>">
                            <a href="resimkategori/<?php echo seo($blogcek['resimkategori_baslik']) ?>/<?php echo $blogcek['resimkategori_id'] ?>"
                               class="blog-card__image__link"><span
                                        class="sr-only"> <?php echo mb_substr($blogcek['resimkategori_baslik'], 0, 50) ?></span>
                                <!-- /.sr-only --></a>
                        </div><!-- /.blog-card__image -->
                        <div class="blog-card__content">
                            <h3 class="blog-card__title mt-4"><a
                                        href="resimkategori/<?php echo seo($blogcek['resimkategori_baslik']) ?>/<?php echo $blogcek['resimkategori_id'] ?>">
                                    <?php echo mb_substr($blogcek['resimkategori_baslik'], 0, 50) ?></a></h3>
                            <!-- /.blog-card__title -->
            ...
            <div class="col-12">
                <ul class="post-pagination">
                    ...
                    while ($s < $toplam_sayfa) {
                        ...
                        <?php } else { ?>
                            <li>
                                <a href="galerikategori.php?sayfa=<?php echo $s ?>"><?php echo $s ?></a>
                            </li>
                        <?php }
                    } ?>
